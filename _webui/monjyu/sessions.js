...
function get_sessions_all() {
    ...
    $.ajax({
        url: $('#core_endpoint').val() + '/get_sessions_all',
        type: 'GET',
        ...
        success: function(data) {
            ...
            if (currentMaxUpdTime !== lastMaxUpdTime) {
                ...
                for (var key in data) {
                    ...
                    var dt = data[key];
                    var inpTime = dt.inp_time ? dt.inp_time.split(' ')[1] : '';
                    // 入力時間の表示用
                ...
                // ゼブラ配色を適用
                $('#sessions tr').each(function(index) {
                    // テーブルの行に交互に背景色を設定する
                    if (index % 2 === 0) {
        ...
        error: function(xhr, status, error) {
            console.error('get_sessions_all error:', error);
        }
    });
}
...
$(document).ready(function() {
    // 情報を取得する
    get_sessions_all();
    setInterval(get_sessions_all, 5000);
});
