...
try:
    import     認証済_google検索
    ggSearch = 認証済_google検索._class()
except:
    try:
        #loader = importlib.machinery.SourceFileLoader('認証済_google検索.py', '_extensions/monjyu/認証済_google検索.py')
        loader = importlib.machinery.SourceFileLoader('認証済_google検索.py', '_extensions/function/認証済_google検索.py')
        認証済_google検索 = loader.load_module()
        ggSearch  = 認証済_google検索._class()
    except:
        print('★"認証済_google検索"は利用できません！')
...
class _class:
    ...
    def __init__(self, ):
        self.version   = "v0"
        self.func_name = "addin_google"
        self.func_ver  = "v0.20240831"
        self.func_auth = "u+K7zj3p/kaJ7Kxvh0qbaEtftPUf1Nlz8z96TA2QYPE="
...
""",
    ...
    def func_proc(self, json_kwargs=None, ):
        ...
        # google 検索実行
        try:
            if (ggSearch is not None):
                dic = {}
                dic['runMode']  = self.runMode
                dic['search_text'] = search_text
                json_dump = json.dumps(dic, ensure_ascii=False, )
                res_json = ggSearch.func_proc(json_dump)
