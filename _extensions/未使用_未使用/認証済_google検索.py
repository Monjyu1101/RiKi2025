...
url2text = None
try:
    import     認証済_url_to_text
    url2text = 認証済_url_to_text._class()
except:
    try:
        loader = importlib.machinery.SourceFileLoader('認証済_url_to_text.py', '_extensions/monjyu/認証済_url_to_text.py')
        addin_url = loader.load_module()
        url2text  = addin_url._class()
    except:
        print('★認証済_url_to_textは利用できません！')

class _class:

    def __init__(self, ):
        self.version   = "v0"
        self.func_name = "googleSearch_to_text"
        self.func_ver  = "v0.20240831"
        self.func_auth = "22LwHgEwMgoGICogurbG7e3pGDUufYu/g6lGeBM4q0nZuK8/42ZB3Y+hqQ6txX0h"
